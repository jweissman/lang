  ...
  context 'DSL' do
    describe 'token' do
      it 'creates a token definition' do
        expect(Numbers.tokens.length).to eq(2)
    ...
    describe 'production' do
      it 'can match tokens' do
        expect(Numbers.parse(input_string: '1234')).to eq([
          :term, [ :factor, [ :value, [ :integer_literal, '1234' ]]]
        ])
      end

      it 'can match patterns' do
        expect(Numbers.parse(input_string: '1+2-3')).to eq(
          [:term, [:factor, [:value, [:integer_literal, "1"]]],
           [:term_prime,
            [:add, [:operator, "+"]],
            [:term,
             [:factor, [:value, [:integer_literal, "2"]]],
             [:term_prime,
              [:sub, [:operator, "-"]],
              [:term, [:factor, [:value, [:integer_literal, "3"]]]]]]]]
        )
      end
    end
  end
  ...
  context 'arithmetic' do
    it 'should match integers' do
      expect(Numbers.parse(input_string: '12345')).to eq(
        [ :term, [ :factor, [ :value, [:integer_literal, '12345']]]]
      )
    end
    ...
    it 'should match integer plus integer' do
      expect(Numbers.parse(input_string: '1+2')).to eq(
        [ :term,
           [ :factor, [ :value, [ :integer_literal, "1" ]]],
           [:term_prime,
            [:add, [:operator, "+"]],
            [:term, [:factor, [:value, [:integer_literal, "2"]]]]
        ]]
      )
    end
